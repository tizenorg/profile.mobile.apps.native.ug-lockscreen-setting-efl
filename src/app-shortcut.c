...
static char *gl_option_text_get_cb(void *data, Evas_Object *obj, const char *part)
{
    ...
    {
        if (get_app_shortcut_state())
            snprintf(buf, sizeof(buf), "<font_size=30>%s</font_size>", "Select which app shortcut you want to use on the lock screen");//TODO: add IDS
        else
            snprintf(buf, sizeof(buf), "<font_size=30>%s</font_size>", "Use app shortcut on the lock screen");//TODO: add IDS
